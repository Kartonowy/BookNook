...
export async function scrapeSK(userInput) {
	let skArray = [];
	await axios
		.get(`https://www.swiatksiazki.pl/catalogsearch/result/?q=${userInput}`)
		.then((response) => {
			const $ = cheerio.load(response.data);
			$(".product-items .item").each((i, elem) => {
				if (
					$(elem).find("div.product-item-type").text().split("|")[0].trim() ===
					"Książki"
				) {
					let title = $(elem).find("strong a").text().trim();
					let author = $(elem).find("span.product-item-author").text().trim();
					let price = $(elem)
						.find(".special-price span.price-wrapper span.price")
						.text();
