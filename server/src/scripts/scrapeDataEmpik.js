...
export async function scrapeEmpik(userInput) {
	...
	await axios
		.get(`https://www.empik.com/szukaj/produkt?q=${userInput}&qtype=basicForm/`)
		.then(async (response) => {
			const $ = cheerio.load(response.data);
			$(".search-content .search-list-item").each((i, elem) => {
				if (
					$(elem).find(".category-line .ta-product-category").text().trim() ===
					"Książki"
				) {
					let title = $(elem).find("h2 a").text().trim();
					let author = $(elem).find(".smartAuthorWrapper").text().trim();
					let price = $(elem).find(".price").text().trim();
					let link = `https://www.empik.com${$(elem)
						.find(".product-title a")
						.attr("href")}`;
					let type = $(elem)
						.find(".category-line .ta-product-carrier")
						.text()
						.split(",")[0]
						.trim();
